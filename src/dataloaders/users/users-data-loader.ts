...
class UsersDataLoader {
  ...
  private async loadById (ids: readonly number[]): Promise<Array<PublicUserModel | null>> {
    if (ids.length === 1) {
  ...
  private async loadByUsername (usernames: readonly string[]): Promise<Array<PublicUserModel | null>> {
