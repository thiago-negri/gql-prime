...
const usersPublicById = new CacheKey("users.public.byId", {
  ...
  resolver: async ({ usersDataLoader }, args: { id: number }) =>
    await usersDataLoader.findById(args.id),
});
...
const usersPublicByUsername = new CacheKeyComposite("users.public.byUsername", {
  ...
  ttlInSeconds: 60,
  inMemoryTtlInSeconds: 10,
  resolver: async ({ usersDataLoader }, args: { username: string }) =>
    await usersDataLoader.findByUsername(args.username),
